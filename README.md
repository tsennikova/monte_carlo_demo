...
```
dbx execute --cluster-name=<name of interactive cluster> --job=monte_carlo_demo-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=monte_carlo_demo-sample-integration-test --files-only
dbx launch --job=monte_carlo_demo-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `monte_carlo_demo/__init__.py` file and tag the current code version:
```
